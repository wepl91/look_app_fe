        ...
        discount.services.forEach( discountedService =>{
          if(this.state.selectedServices.includes(discountedService.id)){
            ...
            if(discount.type == 'POINT'){
              list.push(
                <Panel className="has-text-centered mr-3 ml-3 mt-1 has-text-left" invert color="success" style={{ padding: '2px' }}>
                  <Text size="md" weight="medium" multiline>{ `${this.getText('Promoción ')} "${ discount.name }" ${this.getText('en ')} ${discountedService.name}: ${ this.getText('los puntos del turno se multiplican por ') } ${ discount.pointFactor}` }</Text>
                </Panel>)
            }
          }
        })
